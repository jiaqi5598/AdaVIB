...
from minigpt4.datasets.builders.image_text_pair_builder import PopeBuilder
...
class MiniGPT4EvalModule(pl.LightningModule):
    ...
    def __init__(self, cfg):
        ...
        # self.pred_target = defaultdict(list)
        self.preds: List = []
        ...
        self.dataset_object = PopeBuilder(self.model, cfg_path=cfg.cfg_path)  # cfg -> cfg_path
    ...
    def get_dataset(self):
        ...
        datasets = self.dataset_object.build_datasets()
    ...
    def test_dataloader(self) -> DataLoader:
        return self.get_dataloader(batch_size=self.cfg.run_cfg.batch_size_eval)

    def forward(self, **kwargs):

        return self.model(**kwargs)  # self.model.forward
    ...
    def get_context_embeds(self, image, question):
        ...
        prompt_before = prompt_segs[0]
        prompt_mid, prompt_after = prompt_segs[1].split('<question>')

        seg_tokens = [
            self.model.llama_tokenizer(
                seg, return_tensors="pt", add_special_tokens=i == 0).to(self.device).input_ids
            for i, seg in enumerate([prompt_before, prompt_mid, prompt_after])
        ]
        ...
        seg_embs_mid = seg_embs[1].repeat(image_embs.size(0), 1, 1)
        seg_embs_after = seg_embs[2].repeat(image_embs.size(0), 1, 1)

        question = self.model.llama_model.model.embed_tokens(question).to(image.device)

        context_embs = torch.cat([seg_embs_before, image_embs, seg_embs_mid, question, seg_embs_after], dim=1)
    ...
    def _generative_step(self, batch: dict):
        ...
        context_embs = self.get_context_embeds(image, question)
        ...
        question_text = self.model.llama_tokenizer.batch_decode(question, add_special_tokens=False)

        assert len(image_ids) == len(output_text)

        output_binary = self.post_process_output(output_text)

        base_metrics = {}
        base_metrics.update(output_binary=output_binary, labels=labels, output_text=output_text, image_ids=img_ids, question_text=question_text)
    ...
    def on_test_epoch_end(self):
        ...
        outputs_binary = flatten_list([x["output_binary"] for x in outputs])
        labels = flatten_list([x["labels"] for x in outputs])
        output_text = flatten_list([x["output_text"] for x in outputs])
        image_ids = flatten_list([x["image_ids"] for x in outputs])
        question_text = flatten_list([x['question_text'] for x in outputs])

        pos = 1
        neg = 0
        yes_ratio = outputs_binary.count(pos) / len(outputs_binary)

        TP, TN, FP, FN = 0, 0, 0, 0
        for pred, label in zip(outputs_binary, labels):
        ...
        for i, ot in enumerate(output_text):
            self.preds.append(
                {
                    "id": image_ids[i],
...
def add_args():
    ...
    parser = argparse.ArgumentParser(description="POPE_Eval")

    parser.add_argument("--cfg_path", required=False, help="path to configuration file.",
                        default="minigpt4_configs/minigpt4_pope_eval.yaml")
