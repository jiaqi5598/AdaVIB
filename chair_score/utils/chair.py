...
class CHAIR(object):
    ...
    def __init__(self, imids, coco_path, synonyms_dir):
        ...
        synonyms_file = os.path.join(synonyms_dir, "synonyms.txt")
        synonyms = open(synonyms_file).readlines()
    ...
    def compute_chair(self, cap_file):
        ...
        correct_mentioned_word_count = 0.
        gt_word_count = 0.
        coco_word_count = 0.
        ...
        for i, cap_eval in enumerate(caps):
            ...
            node_words_dedup = list(set(node_words))
            ...
            cap_dict = {'image_id': cap_eval['image_id'], 
                        ...
                        'mscoco_generated_words_dedup': node_words_dedup,
                        'hallucination_idxs': [], 
                        'words': raw_words 
                        }
            ...
            for node_word_dedup in node_words_dedup:
                if node_word_dedup in gt_objects:
                    correct_mentioned_word_count += 1
        ...
        cover = correct_mentioned_word_count / gt_word_count
        output['overall_metrics'] = {'CHAIRs': chair_s, 'CHAIRi': chair_i, 'cover': cover}
...
def load_generated_captions(cap_file):
   ...
   caps = []
   imids = []
   with open(cap_file, mode="r", encoding="utf-8") as rc:
       for line in rc.readlines():
        _cap_line = eval(line)
        img_id = _cap_line['id']
        img_id = int(img_id.split('_')[-1])
        caption = _cap_line['model']
        caps.append({
            "image_id": img_id,
            "caption": caption,
        })
...
def save_hallucinated_words(cap_file, cap_dict):
    ...
    head, tail = os.path.split(cap_file)
    tail_output = "hallucinated_words_{}".format(tail)
    tail_score = "chair_scores_{}".format(tail)
    write_file_score = os.path.join(head, tail_score)
    write_file_output = os.path.join(head, tail_output)

    output_score = cap_dict['overall_metrics']
    with open(write_file_score, "w", encoding="utf-8") as fpw_score:
        json.dump(output_score, fpw_score)

    cap_dict.pop('overall_metrics')
    with open(write_file_output, "w", encoding="utf-8") as fpw_output:

        for line in cap_dict['sentences']:
            _line_s = json.dumps(line)
            fpw_output.write(_line_s + "\n")
...
if __name__ == '__main__':
    ...
    evaluator = CHAIR(imids, args.annotation_path, args.synonyms_dir)
